...
// Export menuItems so it can be used in MobileDrawer
export const menuItems = [
  ...
  { path: '/purchase-orders', name: 'Purchase Orders', icon: IoTicketOutline },
  { path: '/reports', name: 'Reports', icon: IoBarChartOutline },
  { path: '/notifications', name: 'Notifications', icon: IoNotificationsOutline },
  { path: '/users', name: 'Users', icon: IoPersonAddOutline },
  { path: '/settings', name: 'Settings', icon: IoSettingsOutline },
]
...
const Sidebar = ({ isOpen, onToggle }) => {
  return (
    <aside className={`fixed left-0 top-0 z-30 h-screen bg-white dark:bg-gray-800 shadow-lg transition-all duration-300 ${isOpen ? 'w-64' : 'w-20'}`}>
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between px-4 py-4 border-b dark:border-gray-700">
          {isOpen ? (
            <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors hidden lg:block"
          >
            {isOpen ? <IoChevronBackOutline className="w-5 h-5" /> : <IoChevronForwardOutline className="w-5 h-5" />}
          </button>
        </div>
